...
`src/notion_export/` は以下のランタイム依存を要求する:

- `markdown-it-py>=3.0`: Markdown トークン化
- `requests>=2.31`: Notion REST API 呼び出し
...
- **`src/market_pipeline/`**: 市場データの収集・分析パイプラインに直接関わるもの（J-Quants/yfinance/EDINET 取得、分析アルゴリズム、ニュース配信など）
- **`src/notion_export/`**, **`src/market_reader/`**, **`src/technical_tools/`** のような独立パッケージ: パイプライン外の責務（外部サービスへのエクスポート、Jupyter 向け API、レポートビルダーなど）。`src/` 直下に並列配置し、`market_pipeline` への依存は最小に保つ（`market_pipeline.config` / `market_pipeline.master` 程度に限定）

判断基準: 「launchd でスケジュール実行されるパイプラインに組み込むか」が分かれ目。一度限り/手動/イベント駆動の処理は独立パッケージにする。

## ドキュメント記述方針

### 対象読者の使い分け

- **`README.md`**: プロジェクト利用者向け。セットアップ・使用方法・運用上の注意を中心にユーザー視点で記述する
- **`CLAUDE.md`**: Claude Code エージェント向け。実装パターン・コードサンプル・モジュール構成等エージェントが作業に必要な情報を中心に記述する
- **`docs/core/*.md`**: 設計者・開発者向けの公式ドキュメント（アーキテクチャ・API 仕様・開発ガイドライン等）

### 重要な運用注意の冗長記述ルール

以下に該当する重要事項は、対象読者が異なっても `README.md` と `CLAUDE.md` の **両方** に冗長でも記述する:

- **外部公開・共有設定に関するセキュリティ注意**（例: Notion 親ページの共有設定はワークスペース内限定を推奨）
- **データ消失リスクのある操作**（例: マイグレーションスクリプトの dry-run 推奨、自動アーカイブ動作）
- **API 利用上の前提条件**（例: 認証情報の取得方法、レート制限）
- **障害時のリカバリ手順**

利用者・エージェントどちらの導線から入っても見落とさせないことを優先する。
