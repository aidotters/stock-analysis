...
- **銘柄ニュース調査 (`/research-stock-news`):**
    - 特定銘柄のニュース・適時開示・IR情報を包括的に調査するClaude Codeスキルです。
    - 四季報適時開示ページ、四季報銘柄ページのニュースタブ、WebSearchから情報を収集し、レポートを `docs/reports/stocks/` に出力します。
...
- **`chart_classification.py`**: 株価チャートのパターンを分類するロジックです。短期は累積ウィンドウ（直近20/60/120/240日）、中長期は期間スライスウィンドウ（240-480/480-1200/1200-2400/2400-4800日前）でチャート形状を抽出し、「上昇」「下落」「もみ合い」などのパターンに分類します。対数正規化により急騰・急落による歪みを軽減し、NaN値はdropna+50%最低データ閾値で対処します。相関スコアが低い場合（r < 0.3）でもベストマッチのラベルを返し、スコアで信頼度を判断できます。バッチ処理による高性能な分析を提供します。結果は `data/analysis_results.db` の `classification_results` テーブルに保存されます。
...
├── output/              # 分析結果のExcelファイル、銘柄分析レポートなどを格納
│   └── reports/stocks/  # 銘柄詳細分析レポート（analyze-stock出力、タイムスタンプ付きディレクトリ）
...
3.  **依存ライブラリのインストール:**
    ...
    # チャートPNG出力が必要な場合（オプション）
    uv sync --extra chart-export
    ```
...
- **日次分析フロー:**
    ...
    ```bash
    # 全モジュール実行（完了後にIntegrated Analysisも自動実行）
    uv run python scripts/run_daily_analysis.py

    # 全モジュール実行（Integrated Analysisは起動しない）
    uv run python scripts/run_daily_analysis.py --no-chain

    # 特定モジュールのみ実行
    uv run python scripts/run_daily_analysis.py --modules hl_ratio rsp --no-chain

    # yfinanceバリュエーション取得のみ（ローリング更新、デフォルト150銘柄/日）
    uv run python scripts/run_daily_analysis.py --modules yfinance_valuation --no-chain
    ```
...
| スクリプト | 実行タイミング | 説明 |
|-----------|--------------|------|
| `run_daily_jquants.py` | 平日 18:00 | J-Quants株価取得 → `run_daily_analysis.py` → `integrated_analysis2.py` をチェーン実行 |
| `run_weekly_tasks.py` | 土曜 06:00 | 財務諸表取得 + 統合分析 |
| `run_monthly_master.py` | 毎月1日 20:30 | マスターデータ更新 |

> **Note:** 以前は `run_daily_analysis.py`（18:30）と `run_adhoc_integrated_analysis.py`（19:00）に個別のlaunchdジョブがありましたが、DB競合回避のためチェーン実行に変更しました。`--no-chain` フラグで個別実行も可能です。

### Slack通知

launchdスクリプトの実行結果をSlack Incoming Webhookで自動通知します。4つのlaunchdスクリプト全てに統合済みです。
...
特定銘柄のニュース・適時開示・IR情報を包括的に調査し、レポートを生成するClaude Codeスキルです。
